...
- [ ] 补齐 SpliceAI 三分类 baseline，但只在当前小样本 test 上运行。
...
- [ ] 明确 proxy 与真实 frozen encoder 结果的区别。
...
- [ ] 保持实验二窗口范围为当前 `pm50/pm100/pm200/pm400`。
...
- [ ] 将 tissue-specific usage 明确为小样本 case study。
...
- [ ] Borzoi / AlphaGenome 只保留为可选长程 case study。
...
- [ ] 报告中明确 proxy / real case study 边界。
...
- [ ] 拆分 `cryptic_gain` 为 `donor_gain` 与 `acceptor_gain`。
...
- [ ] ClinVar 只做小样本 smoke / case study。
...
- [ ] 将真实 SpliceAI / Pangolin / MMSplice / MaxEntScan 纳入小样本变异对照。
...
- [ ] GTEx sQTL 只做小样本 tissue-specific case study。
...
- [ ] 实现 RNA-FM / RNABERT pseudo-likelihood zero-shot scoring。
...
- [ ] 增加 calibration curve 与多阈值 Top-k / Enrichment。
...
- [ ] 增加 ClinVar smoke case 的 variant delta profile。
...
- [ ] 增加 RNA-FM / RNABERT attention 可视化。
...
- [ ] 支持 Nucleotide Transformer 或 RNAErnie 的小样本对照。
...
- [ ] RNA-FM / RNABERT fine-tuning 仅作为可选小样本实验。
