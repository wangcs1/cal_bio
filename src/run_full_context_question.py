...
from src.utils import PROJECT_ROOT, crop_center, engineered_signal_features, ensure_dirs, write_dataframe
...
def run_local_motif_sufficiency(tables: Path, figures: Path) -> pd.DataFrame:
    rows = []
    train = pd.read_csv(PROJECT_ROOT / "data/splits/train_pm400.csv")
    test = pd.read_csv(PROJECT_ROOT / "data/splits/test_pm400.csv")
...
def run_context_length_ablation(tables: Path, figures: Path) -> pd.DataFrame:
    ...
    for flank in (10, 50, 100, 200, 400):
        source = pd.read_csv(PROJECT_ROOT / "data/processed/splice_sites_pm400.csv")
        current = source.copy()
...
def run_variant_stratified_summary(tables: Path, figures: Path) -> pd.DataFrame:
    scores = pd.read_csv(PROJECT_ROOT / "results/tables/experiment_3A_artificial_variant_scores.csv")
    top_models = [
...
def run(tables: Path | None = None, figures: Path | None = None) -> dict[str, pd.DataFrame]:
    tables = PROJECT_ROOT / "results/tables" if tables is None else tables
    figures = PROJECT_ROOT / "results/figures" if figures is None else figures
