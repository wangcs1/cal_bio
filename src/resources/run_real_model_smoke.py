...
def ensure_clinvar_smoke(limit: int = 5) -> None:
    subprocess.run(
        [sys.executable, "-m", "src.resources.make_clinvar_smoke", "--limit", str(limit)],
        cwd=PROJECT_ROOT,
        check=True,
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run real-model smoke tests on local raw resources.")
    parser.add_argument("--mmsplice-worker", action="store_true", help=argparse.SUPPRESS)
    args = parser.parse_args()

    if args.mmsplice_worker:
