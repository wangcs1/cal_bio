...
from src.build_synthetic_splice_dataset import DEFAULT_WINDOWS, build_and_write
...
def ensure_inputs(windows: list[int]) -> None:
    missing = [PROJECT_ROOT / f"data/splits/train_pm{window}.csv" for window in windows]
...
def run_multiscale(windows: list[int], random_state: int) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for window in windows:
        train = read_csv(PROJECT_ROOT / f"data/splits/train_pm{window}.csv")
        valid = read_csv(PROJECT_ROOT / f"data/splits/valid_pm{window}.csv")
        test = read_csv(PROJECT_ROOT / f"data/splits/test_pm{window}.csv")
